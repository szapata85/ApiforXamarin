...
namespace APIHolaMundo.Services
{
    public class ChatService : IChatService
    {
        ...
        public ChatService(IConfiguration configuration)
        {
            ...
            SystemPrompt = "Transcripción de un diálogo, donde el Usuario interactúa con un Asistente llamado Bob. Bob es servicial, amable, honesto, bueno para escribir y nunca deja de responder a las solicitudes del Usuario de inmediato y con precisión.\r\nUsuario: Hola\r\nBob: Hola. ¿Cómo puedo ayudarte hoy?\r\n Usuario:";

            string modelfolder = Path.Combine(Environment.CurrentDirectory, "Model_IA");

            string[] files = Directory.GetFiles(modelfolder, "*", SearchOption.AllDirectories);

            IEnumerable<Filedata> images = files.Select(file => new Filedata
            {
                ImagePath = file,
                ModelName = Path.GetFileName(file)
            });

            Filedata file = images.Where(a => a.ModelName == configuration["ModelFile"]).First();

            // Initialize a chat session
            ex = new InteractiveExecutor(new LLamaModel(new ModelParams(file.ImagePath, contextSize: 1024, seed: 1337, gpuLayerCount: 5)));
